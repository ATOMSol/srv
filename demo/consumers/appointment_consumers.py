...
class IndexPageConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        # Validate user
        try:
            self.user = await sync_to_async(CustomUser.objects.get)(unique_id=self.filter_created_by_id)
    ...
    async def update_index_page(self, event):
        ...
        # Fix: Use user.phone instead of self.phone
        is_relevant = (
            updated_data.get("date") == self.filter_date and
            (
                (self.filter_role == "assigned_to" and updated_data.get("assigned_to") == self.user.phone) or
                (self.filter_role == "created_by" and updated_data.get("created_by") == self.user.phone)
            )
        )
        print(updated_data)
